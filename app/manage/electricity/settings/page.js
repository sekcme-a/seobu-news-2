...
const FIELDS = [
  {
    key: "price_per_kwh",
    label: "kWh당 요금",
    hint: "삼천리 일반용(갑)저압 저압전력",
  },
  {
    key: "gihu",
    label: "기후환경요금",
    hint: "kWh당 / 삼천리 기후환경요금 검색",
  },
  {
    key: "fuel",
    label: "연료비조정액",
    hint: "kWh당 / 삼천리 공지사항 > 연료비조정단가",
  },
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>
          삼천리 일반용(갑)저압 기준 월별 단가 설정
        </p>
      </div>

      {/* 참고 안내 */}
      <div
        className="card p-5 mb-6 text-sm"
        style={{ borderColor: "rgba(108,142,255,0.3)" }}
      >
        ...
        >
          <div>
            • <strong style={{ color: "var(--text)" }}>kWh당 요금</strong>
            {`: 구글 "삼천리 일반용(갑)저압" → 저압전력 요금표 (한전 직접 문의 권장)`}
          </div>
          <div>
            • <strong style={{ color: "var(--text)" }}>기후환경요금</strong>
            {`: 구글 "삼천리 기후환경요금" → 페이지 하단 확인`}
          </div>
          <div>
            • <strong style={{ color: "var(--text)" }}>연료비조정액</strong>
            {`: 구글 "삼천리 공지사항" → "N분기 전기요금 연료비조정단가 산정내역" → 최종 연료비조정단가`}
          </div>
          <div>
            • <strong style={{ color: "var(--text)" }}>전력기금</strong>
            {`: 구글 "전력기금 요율" → 기사/블로그 확인`}
          </div>
          <div>
            • <strong style={{ color: "var(--text)" }}>공공전기요금</strong>
            {`: 전기요금계에 % 적용 (보통 3.7%)`}
          </div>
        </div>
      </div>
            ...
            <thead>
              ...
              >
                ...
                {FIELDS.map((f) => (
                  <th
                    key={f.key}
                    className="text-left py-3 px-3 text-xs uppercase tracking-wider font-medium"
                  ...
                  {FIELDS.map((f) => (
                    <td key={f.key} className="py-2 px-3">
                      <input
                        ...
                        value={item[f.key] || ""}
                        onChange={(e) =>
                          handleChange(idx, f.key, e.target.value)
                        }
                        placeholder="0"
                      />
                    </td>
